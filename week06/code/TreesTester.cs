public static class TreesTester
{
    ...
    private static void InsertMiddle(int[] sortedNumbers, int first, int last, BinarySearchTree bst)
    {
        ...
        // Recursively insert elements into left and right subtrees
        InsertMiddle(sortedNumbers, first, middleIndex, bst);
        if (!bst.Contains(sortedNumbers[sortedNumbers.Length - 1]))
        {
            if (middleIndex < (sortedNumbers.Length - 1) / 2)
            {
                InsertMiddle(sortedNumbers, (sortedNumbers.Length - 1) / 2, sortedNumbers.Length - 1, bst);
            }
            else
            {
